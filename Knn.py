...
X = data_extracts.data_trasnform('beta', 'beta')
y = np.zeros(X[0].shape[0]+X[1].shape[0])
y[:X[1].shape[0]] = 1
X = np.append(X[1], X[0], axis=0)

X_temp = data_extracts.data_trasnform('theta', 'beta')
y_temp = np.ones(X_temp[0].shape[0]+X_temp[1].shape[0])
y_temp[:X_temp[1].shape[0]] = 0
X_temp = np.append(X_temp[1], X_temp[0], axis=0)
